...
% The maximum number of independent variables is 4
...
for i=1:numel(stimulus)
    [dim1 dim2 dim3 dim4] = ind2sub(size(stimulus),i);
    conditionItemNum = size(stimulus{dim1,dim2,dim3,dim4},1);
    conditionSeq = [conditionSeq; repmat([dim1 dim2 dim3 dim4],conditionItemNum,1)];
end
...
for i=1:size(conditionSeq,1)
    indexSeq(i) = sub2ind(size(stimulus),conditionSeq(i,1),conditionSeq(i,2),conditionSeq(i,3),conditionSeq(i,4));
end
